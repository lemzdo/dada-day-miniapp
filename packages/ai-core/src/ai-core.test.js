...
test('formal recommendation task metadata is frozen', () => { const task = getTask('recommendation_reason'); assert.equal(task.model, 'qwen-flash'); assert.equal(task.prompt, 'recommendation_reason'); assert.equal(task.validator, 'recommendation_production'); assert.equal(task.promptVariant, 'compressed-v2'); assert.equal(task.stream, true); assert.equal(task.retry, 0); assert.equal(task.timeoutMs, 25000); });
...
test('request body is passed unchanged and raw response is exposed', async () => {
  ...
  assert.equal(result.metadata.promptVersion, 'voice-contract-v2.0-compressed-v2-production-4');
});
