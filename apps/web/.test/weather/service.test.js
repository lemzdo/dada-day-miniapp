...
// src/lib/weather/service.test.ts
...
__export(schema_exports, {
  ...
  users: () => users,
  wardrobeAnalyses: () => wardrobeAnalyses,
  weatherCache: () => weatherCache
});
var uuidArray = customType({
  dataType() {
    return "uuid[]";
  }
});
...
var users = pgTable("users", {
  ...
  styleProfile: jsonb("style_profile").default({}).$type(),
...
var clothes = pgTable(
  "clothes",
  {
    ...
    styleTags: jsonb("style_tags").default([]).$type(),
    seasonTags: jsonb("season_tags").default([]).$type(),
    material: varchar("material", { length: 64 }),
    sceneTags: jsonb("scene_tags").default([]).$type(),
    aiRawResult: jsonb("ai_raw_result"),
    // 用户自定义
    customName: varchar("custom_name", { length: 64 }),
    customCategory: varchar("custom_category", { length: 32 }),
    customTags: jsonb("custom_tags").default([]).$type(),
...
var outfits = pgTable(
  "outfits",
  {
    ...
    scoreExplanations: jsonb("score_explanations").default([]).$type(),
...
var shareRecords = pgTable("share_records", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var wardrobeAnalyses = pgTable(
  "wardrobe_analyses",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
    analysisDate: date("analysis_date").defaultNow().notNull(),
    styleBreakdown: jsonb("style_breakdown").$type(),
    colorBreakdown: jsonb("color_breakdown").$type(),
    categoryCounts: jsonb("category_counts").$type(),
    topUsed: uuidArray("top_used"),
    unusedItems: uuidArray("unused_items"),
    missingSuggestions: jsonb("missing_suggestions").default([]).$type(),
...
var weatherCache = pgTable(
  "weather_cache",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    locationKey: varchar("location_key", { length: 128 }).notNull(),
    targetDate: date("target_date").notNull(),
    weatherData: jsonb("weather_data").notNull().$type(),
...
// src/lib/db/index.ts
var connectionString = process?.env?.["DATABASE_URL"] ?? "postgresql://postgres:postgres@localhost:5432/d1d";
var sql2 = (0, import_postgres2.default)(connectionString, {
  max: 10,
  // 最大连接数
  idle_timeout: 20,
  connect_timeout: 10
...
// src/lib/db/repositories/weather.ts
async function getValidWeatherCache(locationKey, targetDate) {
  const rows = await db.select().from(weatherCache).where(
    and(
      eq(weatherCache.locationKey, locationKey),
      eq(weatherCache.targetDate, targetDate),
      gt(weatherCache.expiresAt, /* @__PURE__ */ new Date())
    )
  ).limit(1);
  const row = rows[0];
  if (!row) return null;
...
async function upsertWeatherCache(data) {
  const rows = await db.insert(weatherCache).values({
...
var DEFAULT_CITY = "\u4E0A\u6D77";
var DEFAULT_CITY_CODE = "101020100";
var CURRENT_TTL_MS = 60 * 60 * 1e3;
var FORECAST_TTL_MS = 3 * 60 * 60 * 1e3;
function createWeatherService(options) {
  async function getCurrentWeatherWithCache2(city = DEFAULT_CITY) {
    const targetDate = getDateString(options.now());
    const cityKey = normalizeLocationKey(city);
    const locationKey = `current:${cityKey}`;
    const cached = await options.cache.get(locationKey, targetDate);
  ...
  async function getDailyForecastWithCache2(city = DEFAULT_CITY, date2 = getDateString(options.now())) {
    const cityKey = normalizeLocationKey(city);
    const locationKey = `forecast:${cityKey}`;
    const cached = await options.cache.get(locationKey, date2);
    if (cached) return cached.weatherData;
    const forecast = await fetchDailyForecast(city, date2, options).catch((error) => {
      console.warn("[weather/forecast] provider fallback:", error);
      return getMockDailyForecast(date2);
    });
    await options.cache.set({
      locationKey,
      targetDate: date2,
  ...
  async function getWeatherSnapshotWithCache2(city, _date) {
    const weather = await getCurrentWeatherWithCache2(city ?? DEFAULT_CITY);
    return {
      temp: weather.temp,
  ...
  return {
    getCurrentWeatherWithCache: getCurrentWeatherWithCache2,
    getDailyForecastWithCache: getDailyForecastWithCache2,
    getWeatherSnapshotWithCache: getWeatherSnapshotWithCache2
  };
}
var defaultWeatherService = createWeatherService({
  cache: {
...
var getCurrentWeatherWithCache = defaultWeatherService.getCurrentWeatherWithCache;
var getDailyForecastWithCache = defaultWeatherService.getDailyForecastWithCache;
var getWeatherSnapshotWithCache = defaultWeatherService.getWeatherSnapshotWithCache;
async function fetchCurrentWeather(city, options) {
  const apiKey = options.env["QWEATHER_API_KEY"];
  if (!apiKey) throw new Error("QWEATHER_API_KEY is not configured");
  const response = await fetchQWeather("/v7/weather/now", {
    location: city,
    key: apiKey
  }, options);
  ...
  return {
    ...
    updateTime: now.obsTime ?? (/* @__PURE__ */ new Date()).toISOString()
  };
}
async function fetchDailyForecast(city, date2, options) {
  const apiKey = options.env["QWEATHER_API_KEY"];
  if (!apiKey) throw new Error("QWEATHER_API_KEY is not configured");
  const response = await fetchQWeather("/v7/weather/3d", {
    location: city,
    key: apiKey
  }, options);
  ...
  const daily = response.daily.find((item) => item.fxDate === date2) ?? response.daily[0];
  return {
    date: daily.fxDate ?? date2,
...
async function fetchQWeather(path, params, options) {
...
function getMockCurrentWeather(city, now) {
  return {
    city,
...
function getMockDailyForecast(date2) {
  return {
    date: date2,
    tempHigh: 26,
    tempLow: 16,
...
function createMemoryCache() {
  ...
  return {
    writes: [],
    async get(locationKey, targetDate) {
      const cached = store.get(`${locationKey}:${targetDate}`);
...
(0, import_node_test.default)("getCurrentWeatherWithCache uses cached weather before provider calls", async () => {
  ...
  await cache.set({
    ...
    expiresAt: /* @__PURE__ */ new Date("2026-05-19T02:00:00.000Z")
  });
  let fetchCalls = 0;
  const service = createWeatherService({
    cache,
    env: { QWEATHER_API_KEY: "test-key" },
    now: () => /* @__PURE__ */ new Date("2026-05-19T01:30:00.000Z"),
    fetcher: async () => {
      fetchCalls += 1;
      return new Response("{}");
    }
  ...
  import_strict.default.equal(weather.temp, 19);
  import_strict.default.equal(weather.weather, "\u5C0F\u96E8");
  import_strict.default.equal(fetchCalls, 0);
});
(0, import_node_test.default)("getWeatherSnapshotWithCache falls back and writes cache without provider key", async () => {
  const cache = createMemoryCache();
  ...
  const service = createWeatherService({
    cache,
    env: {},
    now: () => /* @__PURE__ */ new Date("2026-05-19T08:00:00.000Z"),
    fetcher: async () => {
  ...
  import_strict.default.deepEqual(snapshot, {
    temp: 22,
    humidity: 65,
    weather: "\u591A\u4E91",
    wind: 3,
  ...
  import_strict.default.equal(cache.writes.length, 1);
  import_strict.default.equal(cache.writes[0]?.locationKey, "current:\u4E0A\u6D77");
  import_strict.default.equal(cache.writes[0]?.targetDate, "2026-05-19");
});
(0, import_node_test.default)("getDailyForecastWithCache maps QWeather daily response", async () => {
  const cache = createMemoryCache();
  const service = createWeatherService({
    cache,
    ...
    now: () => /* @__PURE__ */ new Date("2026-05-19T08:00:00.000Z"),
    fetcher: async (url) => {
      import_strict.default.equal(new URL(String(url)).hostname, "weather.example");
  ...
  import_strict.default.equal(forecast.tempHigh, 28);
  import_strict.default.equal(forecast.tempLow, 18);
  import_strict.default.equal(forecast.weatherDay, "\u6674");
  import_strict.default.equal(forecast.precipitation, 0.2);
  import_strict.default.equal(cache.writes[0]?.locationKey, "forecast:101020100");
});
