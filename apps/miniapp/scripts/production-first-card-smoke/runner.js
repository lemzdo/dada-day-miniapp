...
async function runProductionSmoke({ admin, mini, directory, progress = () => {}, invoke = requestFirstCard,
  wait = waitForInvocation, date = new Date().toISOString().slice(0, 10) }) {
  ...
  try {
    ...
    const miss = report.requests.find((request) => request.mode === 'miss');
    ...
    report.timelineComparison = {
      before: null,
      after: Object.fromEntries(report.requests.filter((request) => request.mode === 'miss' || request.mode === 'hit')
        .map((request) => [request.mode, request.timeline || {}])),
      comparable: false,
...
async function main() {
  ...
  if (args[0] !== '--live' || !(args.length === 1 || (args.length === 3 && args[1] === '--date'))) {
    console.log('Usage: pnpm first-card:production-smoke [--date YYYY-MM-DD] (current WeChat login + CloudBase admin login required; writes one cache backup, may delete one exact canonical cache, sends at most 3 production requests).');
    process.exitCode = 2;
    return;
  }
  ...
  try {
    ...
    const report = await runProductionSmoke({ admin, mini, directory, date: args[2],
