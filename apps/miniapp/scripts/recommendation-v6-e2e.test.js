...
const TEST_TODAY_SNAPSHOT_KEY = `${TEST_USER_STORAGE_SCOPE}:today:outfitReturnSnapshot:recommendation-copy-contract-v8`;
const TEST_SCENE_SNAPSHOT_PREFIX = `${TEST_USER_STORAGE_SCOPE}:${encodeURIComponent('today:sceneSnapshot:recommendation-copy-contract-v8')}`;
...
test('precise reset removes only current-user recommendation snapshots', async () => {
  const internals = loadE2eInternals();
  const snapshot = acceptanceSnapshot({ count: 8 });
  const otherUserSnapshotKey = 'd1d:userStorage:v1:develop:cloud:test:user:other-user:today:outfitReturnSnapshot:recommendation-copy-contract-v8';
  const harness = createTodayAcceptanceHarness({
    storageEntries: {
