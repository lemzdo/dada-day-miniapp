...
const { buildRecommendationNarrativePlanV2 } = require('./services/recommendationNarrativePlanV2');
const {
  PRODUCTION_MODEL,
...
function buildStylingPlansWithFirstCard(input) {
  try {
    return assembleStylingPlansWithFirstCard(input);
  } catch (error) {
    // Reuse the existing empty-plan envelope when summary construction fails.
    // No successful or failed plan is regenerated by this fallback.
    const empty = runRecommendationStylingShadowV2Safely({ recommendations: [], telemetrySampleRate: 0 });
    return {
      ...empty,
      diagnostics: {
        ...empty.diagnostics,
        ...
        shadowExecutionCount: input.recommendations.length || 1,
        shadowFailureCount: 1,
        sceneCategory: 'other',
        failureCodes: { [readShadowFailureCode(error)]: 1 },
      },
    };
  }
}

function assembleStylingPlansWithFirstCard({ recommendations = [], firstCardPlan, firstCardFailureCode, scene, weather, recommendationInstanceSeed }) {
  const plans = [];
  const planEntries = [];
  const failureCodes = firstCardFailureCode ? [firstCardFailureCode] : [];
  if (firstCardPlan) {
    plans.push(firstCardPlan);
    planEntries.push({ plan: firstCardPlan, recommendation: recommendations[0] });
  }
  for (let index = 1; index < recommendations.length; index += 1) {
    try {
      ...
      failureCodes.push(readShadowFailureCode(error));
    }
  }
  ...
  return {
    ...
    diagnostics: {
      ...diagnostics,
      status: failureCodes.length === 0 ? 'completed' : plans.length > 0 ? 'partially_failed_open' : 'failed_open',
      shadowFailureCount: failureCodes.length,
      failureCodes: countFailureCodes(failureCodes),
    },
  };
}

function readShadowFailureCode(error) {
  const value = (Array.isArray(error?.validationErrors) ? error.validationErrors[0] : undefined)
    || error?.businessCode || error?.code || error?.name;
...
async function computeProductionRecommendationCore(event, diagnostics = createRecommendationDiagnostics(event), scfContext = {}) {
  ...
  // Allocate the v2 identity before candidate materialization so card0 can
      ...
      onFirstCardMaterialized: ({ recommendation }) => {
        ...
        try {
          firstCardPlan = buildRecommendationNarrativePlanV2(recommendation, {
            scene,
            weather: weatherSnapshot,
            recommendationInstanceId: `${diagnostics.auditId || v2BatchId}:0`,
          });
        } catch (error) {
