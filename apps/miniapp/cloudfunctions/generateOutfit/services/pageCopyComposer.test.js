...
test('composePageCopy separates today reason detail explanation and AI default', () => {
  ...
  assert.equal(copy.todayReason, '米白 T恤和白色运动鞋有呼应，军绿色阔腿裤让整套不至于太淡。');
  assert.equal(copy.detailExplanation, '米白 T恤和白色运动鞋前后呼应，军绿色阔腿裤把颜色压下来一点。放在居家场景里，这套不显得刻意，临时出门也顺。');
...
test('composePageCopy avoids mechanical default phrases', () => {
  ...
  for (const phrase of ['主线', '清楚的亮点', '亮点已经落在', '更稳', '保持简单', '单品和单品', '想再明确一点', '放在一起', '不用多想', '不费心']) {
