...
test('V3 constants and prompt describe human-only JSON output', () => {
  assert.equal(STYLIST_PROMPT_VERSION, 'stylist-prompt-v3');
  assert.equal(STYLIST_REVIEW_VERSION, 'stylist-explanation-v3');
  ...
  assert.match(prompt.system, /不得提及识别、证据、线索、维度、覆盖率/);
...
test('validateStylistExplanationV2 accepts V3 overallComment advice and overwrites metadata', () => {
  ...
  assert.equal(result.overallComment, '整体偏轻松日常，白色和灰色放在一起比较稳定。');
  assert.equal(result.advice, '想更完整，可以让鞋子延续白色或灰色。');
...
test('legacy V2-shaped payload is still accepted but normalized without title or tags', () => {
  const result = validateStylistExplanationV2({
    ...
    promptVersion: STYLIST_PROMPT_VERSION,
    title: '模型标题',
    summary: '整体偏轻松日常，白色和灰色放在一起比较稳定。',
    strengths: [{ text: '白色和灰色放在一起比较稳定。', evidenceCodes: ['COLOR_LIGHT_NEUTRAL_BALANCE'] }],
    tradeoffs: [],
    tip: { text: '想更完整，可以让鞋子延续白色或灰色。', evidenceCodes: ['COLOR_LIGHT_NEUTRAL_BALANCE'] },
    styleTags: ['休闲'],
    confidence: 'high',
  }, evidenceInput(), meta());
  ...
  assert.deepEqual(result.styleTags, []);
  assert.equal(result.tip.text, '想更完整，可以让鞋子延续白色或灰色。');
});
...
test('resolveStylistReviewReuse covers V3 reuse upgrade cooldown and stale write protection', () => {
  const context = { inputDigest: 'new', reviewVersion: STYLIST_REVIEW_VERSION, promptVersion: STYLIST_PROMPT_VERSION, copyPolicyVersion: COPY_POLICY_VERSION };
  const ready = {
    status: 'ready',
    reviewVersion: STYLIST_REVIEW_VERSION,
    promptVersion: STYLIST_PROMPT_VERSION,
