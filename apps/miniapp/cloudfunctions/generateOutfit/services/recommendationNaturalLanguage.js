...
const RELATION_SLOTS = Object.freeze({
  ...
  TOP_ACCENT_WITH_NEUTRAL_BOTTOM: relationSlot('relation.accent-neutral', ({ model }) => (
    `${itemLabel(model, 'top', '上衣')}负责亮色重点，${itemLabel(model, 'bottom', '下装')}用中性色接住`
  )),
  DISTINCT_TOP_BOTTOM_COLOR: relationSlot('relation.distinct-top-bottom', ({ model }) => (
    `${itemLabel(model, 'top', '上衣')}和${itemLabel(model, 'bottom', '下装')}用了两种不同颜色`
  )),
...
const SCENE_VALUE_SLOTS = Object.freeze({
  home: sceneSlot('scene.home-direct', '宅家时可以直接这样穿'),
  work: sceneSlot('scene.work-direct', '日常通勤可以直接这样穿'),
  date: sceneSlot('scene.date-direct', '约会时可以直接这样穿'),
  sport: sceneSlot('scene.sport-direct', '日常轻运动可以直接这样穿'),
});
...
const BENEFIT_SLOTS = Object.freeze([
  ...
  benefitSlot('benefit.sport-complete', ['SPORT_COMPLETE_SET'], ['sport_top', 'sport_bottom', 'sport_shoe'], '上衣、下装和运动鞋都照顾到了活动需要'),
  benefitSlot('benefit.sport-light-set', ['SPORT_LIGHT_ACTIVITY_SET'], ['sport_shoe'], '下装和运动鞋也照顾到了活动需要'),
  ...
  benefitSlot('benefit.sport-dress-shoes', ['SPORT_DRESS_SHOES'], ['dress', 'sport_shoe'], '运动鞋能接住裙装，也照顾到了活动需要'),
]);
...
function buildNaturalTodayCopyPlan(model = {}, relation = {}) {
  ...
  if (sceneDefinition) clauses.push(buildSceneClause(sceneDefinition, model, relation, qualification));
  const benefitDefinition = BENEFIT_SLOTS.find((entry) => entry.reasonCodes.includes(qualification.reasonCode));
  const benefitClause = benefitDefinition
...
function relationSlot(id, render) { return Object.freeze({ id, slot: 'relation', render }); }
function detailSlot(id, render) { return Object.freeze({ id, slot: 'relation', render }); }
function sceneSlot(id, text) { return Object.freeze({ id, slot: 'scene_value', text }); }
function benefitSlot(id, reasonCodes, requiredFacts, text) {
  return Object.freeze({ id, slot: 'benefit', reasonCodes: Object.freeze(reasonCodes), requiredFacts: Object.freeze(requiredFacts), text });
}
