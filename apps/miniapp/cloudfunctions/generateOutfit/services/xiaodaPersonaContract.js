const XIAODA_PERSONA_VERSION = 'xiaoda-persona-v6';
...
const ALGORITHM_CHINESE_PATTERNS = Object.freeze([
  ...
  /(?:省得|不用)再想(?:外层|另一套|配色)/u,
  /临时下楼.{0,12}(?:沿用|接着穿|重新换)/u,
  /下装保留自己的颜色/u,
  /(?:用了|选了)同一个颜色/u,
  /颜色(?:接得上|连续|不会.{0,6}断开)/u,
  /主色.{0,6}(?:统一|连起来)/u,
  /(?:其他|剩下)(?:位置|地方).{0,10}(?:中性色|颜色|简单)/u,
  /(?:接住|撑起|托住).{0,10}(?:日常感|利落感|颜色|重点)/u,
  /把(?:日常感|利落感|颜色|重点).{0,8}(?:撑起来|接住|托住)/u,
  /把颜色变化留在衣服上/u,
  /(?:单品|衣服|颜色).{0,8}都比较安静/u,
  /^都比较安静$/u,
  /搭配有照应/u,
  /先把日常这一身搭好/u,
  /(?:颜色|图案|单品).{0,10}(?:支撑关系|承担|形成支撑)/u,
  /(?:鞋子|上衣|下装|外套).{0,8}接住(?:上衣|下装|鞋子|颜色|日常感)?/u,
  /(?:中间|下装).{0,8}(?:隔开|隔着).{0,8}(?:两块|颜色|上衣|鞋子|上下)/u,
  /(?:上衣|下装|鞋子|衣物).{0,8}连成(?:整体|一体)/u,
  /(?:连续感|颜色连续).{0,8}(?:断开|截断|延续)/u,
  /(?:颜色|色调).{0,8}(?:截断视线|关注点)/u,
  /穿在上下两端/u,
  /视线/u,
  /(?:上下两端|拉回两端)/u,
  /(?:中间|下装|裤子).{0,8}(?:隔开|隔着)/u,
  /(?:腿部到脚下|下半身|颜色).{0,10}(?:不断开|连贯|连续)/u,
  /中间(?:的|这块)?.{0,8}(?:更)?集中/u,
  /中间.{0,8}(?:留出|保留).{0,8}(?:颜色|明暗)?变化/u,
]);
